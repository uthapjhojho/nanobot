...
class OpenAICompatProvider(LLMProvider):
    ...
    def _setup_env(self, api_key: str, api_base: str | None) -> None:
        ...
        if not spec or not spec.env_key:
            return
        if spec.is_gateway:
            # Gateways require explicit override: use direct assignment to force
            # the provided credentials, since gateways proxy to multiple backends
            os.environ[spec.env_key] = api_key
        else:
            # Standard providers use setdefault to avoid overriding existing env vars
            # (e.g., if ANTHROPIC_API_KEY is already set, respect that)
